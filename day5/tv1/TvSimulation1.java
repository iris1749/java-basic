...
abstract class 표준Tv { // 주상클래스는 객체화하여 사용할 수 없다. 반드시 상속받아 오버라이딩 해서 사용해야 한다.
    abstract void 켜기(); //주상메서드

    abstract void 끄기();
}

class 샤오미Tv extends 표준Tv { // 표쥰Tv 에게 물려받은 켜기 메서드가 내용이 없는 추상메서드이기 때문에 오버라이딩 하지 않으면 쓸수 없음.
    public void 켜기() {
